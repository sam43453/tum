...
*/attack* `<IP>` `<PORT>` `<TIME>` `<THREADS>` - Start an attack (Admin only)
*/avps* `<IP>` `<USER>` `<PASSWORD>` - Add a new VPS (Admin only)
*/upload* - Upload a file to a VPS (Admin only)
*/ls* `<IP>` - List files on a VPS (Admin only)
*/delete* `<IP>` `<FILE>` - Delete a file on a VPS (Admin only)
*/terminal* `<IP>` `<COMMAND>` - Execute a command on a VPS (Admin only)
*/chmod* `<IP>` - Make all files executable on a VPS (Admin only)

⚠️ *Admin Commands* are restricted to authorized users only.
"""
...
def handle_attack(chat_id, command):
    ...
    if threads > 100:
        send_message(chat_id, "🚫 **Maximum threads is 100!**")
        return
